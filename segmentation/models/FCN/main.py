...
def train(e):
  ...
  for i, (input, target, _) in enumerate(train_loader):
    optimiser.zero_grad()
    input, target = Variable(input.cuda(non_blocking=True)), Variable(target.cuda(non_blocking=True))
...
def test(e):
  ...
  for i, (input, _, target) in enumerate(val_loader):
    input, target = Variable(input.cuda(non_blocking=True), volatile=True), Variable(target.cuda(non_blocking=True), volatile=True)
